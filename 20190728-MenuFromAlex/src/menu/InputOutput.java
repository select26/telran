...
public interface InputOutput {
	...
	default Integer inputInteger(String prompt, Integer minValue, Integer maxValue) {
		return inputObject(prompt, String.format("It's not number in range [%d-%d]", minValue, maxValue), s -> {
			try {
				Integer res = Integer.parseInt(s);
	...
	default Double inputDouble(String prompt) {
		return inputObject(prompt, "It's not  number", s -> {
			try {
	...
	default Long inputLong(String prompt) {
		return inputObject(prompt, "It's not integer(long) number", s -> {
			try {
